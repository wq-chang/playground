...
public class UserEventServiceImpl implements UserEventService {
  ...
  private EventMessage mapToUserEventMsg(Event event) {
    UserOperation operation = UserOperation.getByKeycloakEventType(event.getType());
    ...
    return new EventMessage(UserEventType.USER_EVENT, operation, event.getUserId(), updatedDetails);
  }
  ...
  private EventMessage mapToUserEventMsg(AdminEvent event) {
    UserOperation operation = UserOperation.getByKeycloakOperationType(event.getOperationType());

    return new EventMessage(UserEventType.ADMIN_EVENT, operation, event.getResourceId(), null);
  }
}
